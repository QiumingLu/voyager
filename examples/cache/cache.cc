...
struct LRUHandle {
  ...
  LRUHandle* next;
  LRUHandle* prev;
  size_t charge;      // TODO(opt): Only allow size_t?
  size_t key_length;
  bool in_cache;      // Whether entry is in the cache.
  size_t refs;      // References, including cache reference, if present.
...
class LRUCache {
 public:
  ...
  Cache::Handle* Insert(const Slice& key, size_t hash,
                        void* value, size_t charge,
                        void (*deleter)(const Slice& key, void* value));
  Cache::Handle* Lookup(const Slice& key, size_t hash);
...
Cache::Handle* LRUCache::Lookup(const Slice& key, size_t hash) {
  port::MutexLock l(&mutex_);
...
// from the hash table.  Return whether e != nullptr.  Requires mutex_ held.
...
class ShardedLRUCache : public Cache {
 private:
  ...
  static size_t Shard(size_t hash) {
