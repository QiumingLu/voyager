...
void ThreadFunc() { printf("tid=%" PRIu64 "\n", port::CurrentThread::Tid()); }
...
TEST(ThreadTest, TestThread) {
  ...
  port::Thread t2(std::bind(ThreadFunc2, 42), "thread2");
  t2.Start();
  t2.Join();

  Foo foo(78.8);
  port::Thread t3(std::bind(&Foo::MemberFunc, &foo), "thread3");
  t3.Start();
  t3.Join();

  port::Thread t4(
      std::bind(&Foo::MemberFunc2, std::ref(foo), std::string("thread4")));
  t4.Start();
  t4.Join();
}
